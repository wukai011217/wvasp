...
main() {
    ...
    if [[ "${CONFIG[command]}" == "1" ]]; then
        logging 1 "WARNING: Command 1 will delete ALL files in target directories!"
        if [[ "${CONFIG[dry_run]}" != true ]]; then
            logging 1 "Use --dry-run to preview what will be deleted."
            sleep 2  # 给用户时间阅读警告
        fi
    fi
    ...
    declare -A COMMAND_MAP=(
        ...
        [1]="Delete all files (DANGEROUS):delete_all_directories"
    )
